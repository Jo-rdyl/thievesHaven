...
function Weather() {
  ...
  const fetchCityWeather = async (event: any) => {
    ...
    try {
      ...
      ${theCityName}&units=metric&appid=51e69e6229683f0c12fea8d17f57063e`);//the url to fetch the weather infomation
      ...
      console.log(JSON.stringify(data,null,2));//Garnage shit that i'll remove
    }
    catch (error) {
...
let SearchBar = async (e:any) =>{ //e is short for event I just like using it
  ...
  const actualData = data.data;//thhis helps with mapping the cities later on
  console.log(data)
